...
# 새 제안서를 추가할 때는 이 리스트에 dict를 추가하면 됩니다
# ─────────────────────────────────────────
PROPOSAL_EXAMPLES = [
    {
        "client": "한국제조(주)",
        ...
        "sections": {
            "executive_summary": "귀사 중간관리자의 리더십 역량 강화를 통해 조직 성과를 극대화하고 세대 간 소통 문화를 정착시키기 위한 맞춤형 2일 과정을 제안드립니다. 본 과정은 진단-이론-실습-적용의 4단계 구조로 설계되어 현업 복귀 후 즉시 활용 가능한 실천 역량을 배양합니다.",
            "needs_analysis": "귀사의 현황을 분석한 결과, 첫째 MZ세대 직원과의 세대 간 가치관 차이로 인한 소통 단절이 팀 성과에 영향을 미치고 있으며, 둘째 목표 설정 및 피드백 역량 부족으로 성과관리 체계가 개인 역량에 의존하는 상황입니다. 본 과정은 이 두 가지 핵심 문제를 집중적으로 해결합니다.",
            "curriculum": [
                "Day 1 오전 | 리더십 스타일 진단 및 자기이해 (4H)",
                "Day 1 오후 | MZ세대 이해와 세대 간 소통 스킬 (4H)",
                "Day 2 오전 | 성과관리와 목표설정(OKR) 실습 (4H)",
                "Day 2 오후 | 코칭 피드백 스킬 & 현업 적용 계획 수립 (4H)"
            ],
            "budget_detail": "강사비 500만원 / 교재·진단도구 150만원 / 장소·식음료 150만원",
            "why_us": "국내 500대 기업 중 120개사 납품 실적, 제조업 특화 강사진 보유, 교육 후 3개월 사후 코칭 무상 제공"
        }
    },
    {
        "client": "미래금융(주)",
        ...
        "sections": {
            "executive_summary": "귀사 신입사원이 금융업의 특수한 고객 서비스 환경에 빠르게 적응하고 전문가로 성장할 수 있도록 온·오프라인을 결합한 3일 블렌디드 과정을 제안드립니다. 사전 온라인 학습으로 기초를 다지고, 집합교육에서 실전 역량을 완성합니다.",
            ...
            "curriculum": [
                "사전학습(온라인) | 금융업 기초·컴플라이언스 이해 (4H, 자율)",
                "Day 1 | 조직문화 이해 및 비즈니스 에티켓 (8H)",
                "Day 2 | 고객 응대 스킬 & 디지털 소통 (8H)",
                "Day 3 | 실전 롤플레이 & 팀 프로젝트 발표 (4H)"
            ],
            "budget_detail": "강사비 700만원 / 온라인 콘텐츠 제작 250만원 / 교재·운영 250만원",
            "why_us": "금융권 온보딩 과정 연간 50회 이상 운영, 자체 LMS 플랫폼 제공, 수료 후 6개월 온라인 복습 콘텐츠 무제한 제공"
        }
    }
]
...
# Streamlit UI
# ─────────────────────────────────────────
st.set_page_config(page_title="기업교육 제안서 생성기", page_icon="📋", layout="wide")
st.title("📋 기업교육 제안서 자동 생성기")
st.caption("AI가 맞춤형 제안서 초안을 자동으로 작성합니다.")

# API Key 입력
with st.sidebar:
    st.header("⚙️ 설정")
    api_key = st.text_input("Anthropic API Key", type="password", help="sk-ant-... 형태의 키를 입력하세요")
    st.divider()
    st.markdown("**📚 학습된 제안서**")
    for ex in PROPOSAL_EXAMPLES:
        st.markdown(f"- {ex['client']} ({ex['topic']})")
    st.divider()
    st.markdown("**➕ 새 예시 추가하려면**")
    st.caption("app.py의 PROPOSAL_EXAMPLES 리스트에 dict를 추가하고 GitHub에 push하세요.")

# 입력 폼
st.subheader("1️⃣ 고객사 정보 입력")
col1, col2 = st.columns(2)
with col1:
    company  = st.text_input("회사명", placeholder="예: (주)삼성전자")
    industry = st.text_input("업종", placeholder="예: IT, 금융, 제조업")
    target   = st.text_input("교육 대상", placeholder="예: 신입사원, 팀장급")
    headcount = st.text_input("교육 인원", placeholder="예: 50명")
with col2:
    topic    = st.text_input("교육 주제", placeholder="예: 리더십 역량 강화")
    fmt      = st.selectbox("교육 형태", ["집합교육(오프라인)", "온라인(비실시간)", "화상교육(실시간)", "혼합형(블렌디드)"])
    duration = st.text_input("교육 기간", placeholder="예: 1일(8H), 2박3일")
    budget   = st.text_input("예산", placeholder="예: 500만원")

needs = st.text_area("핵심 니즈 / 요청사항", placeholder="예: 조직 내 세대 간 갈등 해소, 협업 문화 구축이 필요합니다.", height=100)
...
if st.button("🚀 제안서 생성하기", type="primary", use_container_width=True):
    if not api_key:
        ...
        st.warning("회사명, 업종, 교육 대상, 교육 주제, 핵심 니즈는 필수 입력입니다.")
    else:
        client_info = {
            "company": company, "industry": industry, "target": target,
            "headcount": headcount, "topic": topic, "format": fmt,
            "duration": duration, "budget": budget, "needs": needs
        }
        with st.spinner("AI가 제안서를 작성 중입니다... (약 15~30초)"):
            try:
...
if "proposal" in st.session_state:
    ...
    st.subheader("2️⃣ 생성된 제안서")
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📌 제안 요약", "🔍 니즈 분석", "📅 커리큘럼", "💰 예산", "⭐ 선택 이유"])
    with tab1:
